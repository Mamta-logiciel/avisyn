<!-- Header Part -->
<?php include('header.php'); ?>
<!-- End Header Part -->

<div class="main-wrap">
	<div class="container">
		<!-- Page heading section starts -->
		<section class="heading-wrap">
			<h1 class="title">our services</h1>
			<p class="tag-line">Agile insourcing for fast, flexible drug discovery and development.</p>
		</section>
		<!-- Page heading section ends -->	
	</div>
</div>

<!-- Main container content section starts -->
<div class="content-container">
	<div class="container">
		<section class="content-wrap main-text-cols">

			<!-- Section What is Insourcing -->
			<div class="who-avisyn" id="what-is-insourcing">
				<div class="row">
					<div class="col-md-6 avisyn-text-outer">
						<h2 class="title">what is insourcing?</h2>
						<p>Insourcing is an innovative scientific staffing solution that is ideally suited for the changing stages of drug discovery and development programs.</p>
						<p>Combining the flexibility and expertise of outsourcing to a CRO with the immediate communication, collaboration, and IP security possible with an in-house team, insourcing lets companies hire expert teams of scientists for defined project stages. Simply <a href="contact-project.php">contact us</a> with your project needs, and within weeks we'll have a team up and running at your site.</p>
						<hr class="text-with-border">
						<p><span>Find out how</span> one scientist's insourcing experience inspired him to start AviSyn. <a href="about.php#who-avisyn"><b>READ NOW</b></a></p>
					</div>
					<div class="col-md-6 founder-img-outer">
						<img src="images/insourcing-services.png" class="img-responsive founder-img">
					</div>
				</div>
			</div>

			<!-- Section Insource with Avisyn -->
			<div class="services-mid-banner mid-banner" id="why-insource">
				<div class="banner-inner-content">
					<h4>why insource with avisyn</h4>
					<p>With decades of combined drug discovery and development research in pharma and biotech — both managing and at the bench — the AviSyn team understands the kind of expertise it takes to move compounds through the pipeline. We nurture teams of expert, innovative scientists that can be in place, on site within weeks, and that will stay until your project is complete. Reporting directly to your project leaders while also being further guided and mentored by our experienced management team, AviSyn's scientists bring a wealth of industry experience to your site.</p>
				</div>
			</div>

			<!-- Section Insource cols -->
			<div class="who-avisyn services-cols">
				<div class="row">
					<div class="col-md-6 cols-content" id="expertise-offer">
						<img src="images/expertise-offer.png">
						<h4>what expertise do we offer?</h4>
						<p>Supporting a wide range of drug discovery and development activities, we are currently offering scientific expertise in the following areas:  Synthesis, medicinal chemistry design, computational chemistry, and analytical chemistry.</p>
					</div>
					<div class="col-md-6 cols-content" id="region-serve">
						<img src="images/map.png">
						<h4>what regions do we serve?</h4>
						<p>Currently focused on the San Diego area, we are rapidly expanding our services to other biotech hubs such as San Francisco and Boston. <a href="contact-project.php">Contact us</a> to find out if we can support a project in your region.</p>
					</div>
				</div>
			</div>
		</section>
	</div>
</div>
<!-- Main container content section starts -->

<!-- Footer Part -->
<?php include('footer.php'); ?>
<!-- End Footer Part -->
